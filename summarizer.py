...
def selection_phrases(phrases, scores, ratio):
    """
    Sélectionne les phrases les plus importantes selon leur score.

    Args:
        phrases (list of str): phrases originales.
        scores (list of float): score TextRank de chaque phrase.
        ratio (float): pourcentage de phrases à garder (0.05–0.5).

    Returns:
        list of str: phrases sélectionnées avec ordre original.
    """
    n = len(phrases)
...
def main():
    ...
    parser.add_argument(
        ...
        nargs="+",
        required=True,
        help="Fichier(s) ou dossier(s) .txt à résumer",
    )
    parser.add_argument(
        "--ratio",
        type=float,
        required=True,
        help="Ratio de phrases à conserver (0.05–0.5)",
    )
    parser.add_argument(
        "--mode", choices=["french", "english"], required=True, help="Langue du texte"
    )
    parser.add_argument(
        "--output",
        type=str,
        default=None,
        help="Chemin du fichier de sortie (sinon console)",
    )
    ...
    print(
        f"Nombre total de phrases : {len(phrases_original)} ; termes uniques : {len(vocab)}.\n"
    )
    print(
        f"Construction de la matrice TF-IDF ({len(phrases_original)} × {len(vocab)})… temps écoulé : {duree_tfidf:.3f} s."
    )
    print(
    ...
    # Emplacement du fichier de sortie ou, si pas de --output , afficher le résumé dans la console.
    if args.output:
